...
const Login = () => {
  ...
  const handleGoogleSignIn = useCallback(async (credential) => {
    ...
    // Google callback may return object or string - ensure it's a string
    let credentialString = credential;
    
    if (typeof credential !== 'string') {
      // If it's an object, extract the credential property
      if (credential && typeof credential === 'object') {
        credentialString = credential.credential || credential.id_token || credential;
      }
      
      // If still not a string, convert to string
      if (typeof credentialString !== 'string') {
        credentialString = String(credentialString);
      }
    }
    
    // Validate it looks like a JWT token (has 3 parts separated by dots)
    if (!credentialString || !credentialString.includes('.')) {
      setError('Credential không hợp lệ. Vui lòng thử lại.');
      setIsGoogleLoading(false);
      return;
    }
    
    try {
      const result = await googleLogin(credentialString);
      ...
      setError(err.message || 'Có lỗi xảy ra khi đăng nhập Google');
    } finally {
      setIsGoogleLoading(false);
    }
    ...
    <AuthLayout mode="login">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div ref={googleButtonRef} className="flex items-center justify-center w-full"></div>
          {isGoogleLoading && (
            <div className="absolute inset-0 flex items-center justify-center bg-white bg-opacity-75 z-10">
              <svg className="animate-spin h-5 w-5 text-[#1A66CC]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
              </svg>
            </div>
